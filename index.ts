...
type MemoryCategory = "preference" | "fact" | "decision" | "entity" | "other" | "behavioral";
...
function parseMemoryCategories(value: unknown, fallback: MemoryCategory[]): MemoryCategory[] {
  ...
  const parsed = value
    ...
    .map((item) => {
      if (item === "reflection") {
        rejectRemovedConfigField("autoRecallCategories[]=reflection", "autoRecallCategories[]=behavioral");
      }
      return item;
    })
...
const DIAG_BUILD_TAG = "chronicle-engine-diag-20260308-0058";
...
function isInternalBehavioralGuidanceSessionKey(sessionKey: unknown): boolean {
  return typeof sessionKey === "string" && sessionKey.trim().startsWith("temp:memory-behavioral-guidance");
}
...
const chronicleEnginePlugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    api.logger.info(
      `chronicle-engine@${pluginVersion}: plugin registered ` +
      `(mode: remote-backend, authority: backend-owned)`,
    );
    api.logger.info(`chronicle-engine: diagnostic build tag loaded (${DIAG_BUILD_TAG})`);
    api.logger.info(
      `chronicle-engine: remote backend enabled (${config.remoteBackend?.baseURL || "(missing baseURL)"})`
    );
    // ========================================================================
    // Register Tools
    // ========================================================================
      ...
      },
      {
        ...
        recallGeneric: async (params) => {
          ...
          if (!resolved.hasPrincipalIdentity) {
            api.logger.warn(
              `chronicle-engine: auto-recall skipped remote recall (missing runtime principal: ${resolved.missingPrincipalFields.join(", ")})`
            );
            return [];
          }
    ...
    if (config.autoRecall === true) {
      api.on("before_agent_start", async (event, ctx) => {
        ...
        try {
          ...
          api.logger.warn(`chronicle-engine: auto-recall failed: ${String(err)}`);
        }
      });
    }
    ...
    api.on("agent_end", async (event, ctx) => {
      ...
      if (!resolvedBackendCtx.hasPrincipalIdentity) {
        api.logger.warn(
          `chronicle-engine: transcript append blocked (missing runtime principal: ${resolvedBackendCtx.missingPrincipalFields.join(", ")})`
        );
        return;
      }
      ...
      try {
        ...
        api.logger.info(
          `chronicle-engine: session transcript appended to remote backend (${transcriptItems.length} item(s), appended=${appendResult.appended})`
        );
      } catch (err) {
        api.logger.warn(`chronicle-engine: session transcript append failed: ${String(err)}`);
      }
      ...
      if (config.distill?.enabled === true) {
        ...
        if (userTurnCount > 0 && cadence.crossedBoundary) {
          try {
            ...
            api.logger.info(
              `chronicle-engine: automatic distill enqueued (${job.jobId}, everyTurns=${config.distill.everyTurns}, completedUserTurns=${cadence.completedUserTurns})`
            );
          } catch (err) {
            api.logger.warn(`chronicle-engine: automatic distill enqueue failed: ${String(err)}`);
          }
        }
      }
      ...
      try {
        ...
        api.logger.info(
          `chronicle-engine: auto-capture forwarded to remote backend (${captureItems.length} item(s), mutations=${result.length})`
        );
      } catch (err) {
        api.logger.warn(`chronicle-engine: capture failed: ${String(err)}`);
      }
    });
    ...
    const registerDurableCommandHook = (
      ...
      if (typeof onFn === "function") {
        try {
          ...
          api.logger.warn(
            `chronicle-engine: failed to register ${eventName} via api.on, continue fallback: ${String(err)}`,
          );
        }
      }
      ...
      if (typeof registerHookFn === "function") {
        try {
          ...
          api.logger.warn(
            `chronicle-engine: failed to register ${eventName} via api.registerHook: ${String(err)}`,
          );
        }
      }
      ...
      if (!registeredViaEventBus && !registeredViaInternalHook) {
        api.logger.warn(
          `chronicle-engine: command hook registration failed for ${eventName}; no compatible API method available`,
        );
      }
    };
    ...
    if (config.autoRecallBehavioral?.enabled === true) {
      ...
      api.registerHook("agent:bootstrap", async (event) => {
        ...
        name: "chronicle-engine.auto-recall-behavioral.agent-bootstrap",
      ...
      if (config.autoRecallBehavioral?.beforeResetNote !== false) {
        ...
        const behavioralGuidanceNewHookOptions = {
          name: "chronicle-engine.auto-recall-behavioral.command-new",
          description: "Append behavioral guidance note before /new",
        } as const;
        const behavioralGuidanceResetHookOptions = {
          name: "chronicle-engine.auto-recall-behavioral.command-reset",
    ...
    api.registerService({
      id: "chronicle-engine",
      start: async () => {
        api.logger.info("chronicle-engine: remote backend mode active");
      },
      stop: async () => {
        api.logger.info("chronicle-engine: stopped (remote backend mode)");
      },
    });
  },
};
...
export function parsePluginConfig(value: unknown): PluginConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("chronicle-engine config required");
  }
