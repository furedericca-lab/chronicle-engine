...
3. `src/context/*` is the local prompt-time orchestration layer.
   ...
   - Must not own backend-visible candidate filtering semantics such as category, behavioral kind, score-threshold, age-window, or per-key recall caps.
...
- Canonical backend HTTP/data-plane naming for the behavioral-guidance lane is `/v1/recall/behavioral`, `/v1/debug/recall/behavioral`, `behavioralCount`, `excludeBehavioral`, and behavioral trace/category labels.
- The active LanceDB schema now uses behavioral-facing internal names such as `category=behavioral`, `behavioral_kind`, and `behavioral:` strict-key prefixes.
- Legacy reflection-era LanceDB schemas are intentionally unsupported at runtime and require manual migration or reset before startup.
