...
对于 Docker 部署，现在镜像内部会自带默认 `backend.toml`。推荐做法是不再默认挂载配置文件，而是在 Docker Compose 的 `environment:` 中直接使用嵌套 TOML 路径对应的环境变量名，路径之间用双下划线表示，例如：
...
Docker Compose 常用可配置环境变量：

| 环境变量 | 对应 TOML 键 | 作用 | 期待格式 | 示例 |
|---|---|---|---|---|
| `AUTH__RUNTIME__TOKEN` | `auth.runtime.token` | `/v1/*` 数据面 bearer token | 非空字符串 | `AUTH__RUNTIME__TOKEN=prod-runtime-token` |
| `AUTH__ADMIN__TOKEN` | `auth.admin.token` | `/admin/api/*` 管理面 bearer token | 非空字符串 | `AUTH__ADMIN__TOKEN=prod-admin-token` |
| `SERVER__BIND` | `server.bind` | 容器内监听地址 | `host:port` 字符串 | `SERVER__BIND=0.0.0.0:8080` |
| `SERVER__ADMIN_ASSETS_PATH` | `server.admin_assets_path` | 管理后台静态资源路径 | 绝对路径字符串 | `SERVER__ADMIN_ASSETS_PATH=/usr/local/bin/web/dist` |
| `STORAGE__LANCEDB_PATH` | `storage.lancedb_path` | LanceDB 数据目录 | 绝对路径字符串 | `STORAGE__LANCEDB_PATH=/var/lib/chronicle-engine-backend/lancedb` |
| `STORAGE__SQLITE_PATH` | `storage.sqlite_path` | SQLite job / audit 数据库路径 | 绝对路径字符串 | `STORAGE__SQLITE_PATH=/var/lib/chronicle-engine-backend/sqlite/jobs.db` |
| `LOGGING__LEVEL` | `logging.level` | 后端日志级别 | `trace` / `debug` / `info` / `warn` / `error` | `LOGGING__LEVEL=info` |
| `PROVIDERS__EMBEDDING__BASE_URL` | `providers.embedding.base_url` | embedding provider 基础地址 | URL 字符串 | `PROVIDERS__EMBEDDING__BASE_URL=https://api.openai.com/v1` |
| `PROVIDERS__EMBEDDING__MODEL` | `providers.embedding.model` | embedding 模型名 | 字符串 | `PROVIDERS__EMBEDDING__MODEL=text-embedding-3-small` |
| `PROVIDERS__EMBEDDING__API` | `providers.embedding.api` | embedding provider API 类型 | `openai` / `builtin` 等字符串 | `PROVIDERS__EMBEDDING__API=openai` |
| `PROVIDERS__EMBEDDING__API_KEY` | `providers.embedding.api_key` | embedding provider API key | 非空字符串 | `PROVIDERS__EMBEDDING__API_KEY=sk-...` |
| `PROVIDERS__RERANK__ENABLED` | `providers.rerank.enabled` | 是否启用 provider-backed rerank | 布尔值 `true` / `false` | `PROVIDERS__RERANK__ENABLED=false` |

路径映射规则：

- 用 `TABLE__SUBTABLE__KEY` 映射到嵌套 TOML 键。
- 字符串直接传字符串。
- 布尔值使用 `true` / `false`。
- 数字使用普通整数或浮点字面量。
- 未命中真实配置路径的变量会被忽略。
