...
describe("remote backend shell integration", () => {
  ...
  it("registers management-gated distill/debug tools and forwards existing backend contracts", async () => {
    ...
    const fetchMock = installFetchMock([
      ...
      {
        method: "POST",
        path: "/v1/debug/recall/behavioral",
        reply: () => ({
          rows: [
            {
              id: "refl-debug-1",
    ...
    assert.equal(behavioralDebugCall.path, "/v1/debug/recall/behavioral");
  ...
  it("blocks manual behavioral-category store/update writes before calling the backend", async () => {
    const root = makeTempRoot();
    const fetchMock = installFetchMock([]);
    ...
    const toolCtx = {
      userId: "user-behavioral-write",
      agentId: "agent-behavioral-write",
      sessionId: "session-behavioral-write",
      sessionKey: "agent:agent-behavioral-write:session:stable-behavioral-write",
    };

    const store = harness.instantiateTool("memory_store", toolCtx);
    const storeResult = await store.execute("call-store-behavioral", {
      text: "Do not allow this manual behavioral write",
      category: "behavioral",
    });
    assert.equal(storeResult.details.error, "invalid_param");
    assert.equal(storeResult.details.code, "behavioral_category_reserved");
    assert.match(storeResult.content[0].text, /behavioral-guidance rows are backend-managed/);

    const update = harness.instantiateTool("memory_update", toolCtx);
    const updateResult = await update.execute("call-update-behavioral", {
      memoryId: "123e4567-e89b-12d3-a456-426614174000",
      category: "behavioral",
    });
    assert.equal(updateResult.details.error, "invalid_param");
    assert.equal(updateResult.details.code, "behavioral_category_reserved");
    assert.match(updateResult.content[0].text, /behavioral-guidance rows are backend-managed/);
  ...
  it("uses backend behavioral recall in before_prompt_build and preserves runtime context fields", async () => {
    ...
    assert.equal(fetchMock.calls.length, 1);
    const recallCall = fetchMock.calls[0];
    assert.equal(recallCall.path, "/v1/recall/behavioral");
  ...
  it("keeps behavioral recall fail-open when backend behavioral endpoint fails", async () => {
    const root = makeTempRoot();
