function isBehavioralGuidanceEntry(entry: { category: string; metadata?: string }): boolean {
  return entry.category === "behavioral";
}
...
function extractSectionMarkdown(markdown: string, heading: string): string {
...
function isPlaceholderBehavioralGuidanceSliceLine(line: string): boolean {
...
function normalizeBehavioralGuidanceSliceLine(line: string): string {
  return line
    .replace(/\*\*/g, "")
...
function sanitizeBehavioralGuidanceSliceLines(lines: string[]): string[] {
  return lines
...
export function extractBehavioralGuidanceMappedMemories(behavioralGuidanceText: string): BehavioralGuidanceMappedMemory[] {
  const mappedItems = extractBehavioralGuidanceMappedMemoryItems(behavioralGuidanceText);
  return mappedItems.map(({ text, category, heading }) => ({ text, category, heading }));
}

function extractBehavioralGuidanceMappedMemoryItems(behavioralGuidanceText: string): BehavioralGuidanceMappedMemoryItem[] {
  ...
  return mappedSections.flatMap(({ heading, category, mappedKind }) => {
    ...
    return lines.map((text, ordinal): BehavioralGuidanceMappedMemoryItem => ({ text, category, heading, mappedKind, ordinal, groupSize }));
  });
}
