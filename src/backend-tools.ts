      ...
      async execute(_toolCallId, params) {
        ...
        try {
          ...
          if (!resolved.hasPrincipalIdentity) {
            ...
            api.logger.warn?.(`chronicle-engine: memory_recall skipped (${warning})`);
            return {
    ...
    (toolCtx) => ({
      ...
      async execute(_toolCallId, params) {
        const { text, importance, category } = params as {
          text: string;
          importance?: number;
          category?: WritableMemoryCategory | "behavioral";
        };
...
function manualBehavioralGuidanceWriteSurfaceError(category: unknown) {
  if (category !== "behavioral") return undefined;
  return {
    content: [
      {
        type: "text",
        text: "behavioral-guidance rows are backend-managed and cannot be written manually. Use distill for trajectory-derived knowledge.",
      },
    ],
    details: {
      error: "invalid_param",
