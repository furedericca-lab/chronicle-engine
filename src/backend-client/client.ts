...
export function createMemoryBackendClient(config: MemoryBackendClientConfig): MemoryBackendClient {
  ...
  return {
    async recallGeneric(ctx, input) {
      const body = {
        ...
        ...(Array.isArray(input.categories) && input.categories.length > 0
          ...
          : {}),
        ...(input.excludeBehavioral === true ? { excludeBehavioral: true } : {}),
    ...
    async stats(ctx) {
      const response = await requestJson<{ memoryCount: number; behavioralCount: number; categories: Record<string, number> }>({
        method: "POST",
      ...
      return {
        ...
        behavioralCount: Number(response.behavioralCount || 0),
