...
export async function orchestrateDynamicRecall<T extends DynamicRecallCandidate>(
  ...
  if (injected.length === 0) {
    params.logger?.debug?.(
      `chronicle-engine: ${params.channelName} skipped due to repeated-injection guard (session=${sessionId}, turn=${currentTurn})`
    );
    return undefined;
  }
  ...
  params.logger?.info?.(
    `chronicle-engine: ${params.channelName} injecting ${memoryLines.length} row(s) for session=${sessionId}`
  );
