use crate::{
    ...
    models::{
        clamped_limit, manual_behavioral_guidance_write_error, validate_non_empty, Actor,
        BehavioralMetadata, BehavioralRecallMode, Category, DeleteRequest, DistillArtifact,
        ...
        RecallBehavioralRequest, RecallBehavioralResponse, RecallGenericRequest,
        RecallGenericResponse, RecallGenericRow, ReflectionKind, RetrievalTrace,
        RetrievalTraceKind, RetrievalTraceQuery, RetrievalTraceStage, RetrievalTraceStageStatus,
        RowMetadata, StatsResponse, StoreRequest, StoreResponse, ToolStoreMemory, UpdateRequest,
...
struct MemoryRow {
    ...
    behavioral_kind: Option<ReflectionKind>,
...
struct TableCapabilities {
    ...
    has_behavioral_kind_column: bool,
    has_legacy_reflection_kind_column: bool,
}
...
struct RerankApplyArgs<'a> {
    query: &'a str,
    normalized_query: &'a str,
    query_tokens: &'a [String],
    candidates: &'a mut [ScoredCandidate],
    blend: f64,
    mode: RerankMode,
    diagnostics: bool,
...
impl GenericRecallEngine {
    ...
    async fn rank_candidates(
        ...
        let rerank_summary = self
            .reranker
            .apply(RerankApplyArgs {
                query,
                normalized_query: &normalized_query,
                query_tokens: &unique_query_tokens,
                candidates: &mut rerank_candidates,
                blend: self.settings.rerank_blend,
                mode: self.settings.rerank_mode,
                diagnostics: self.settings.diagnostics,
                candidate_count: selected_for_rerank,
            })
            .await?;
        if let Some(trace) = trace.as_deref_mut() {
...
impl RerankProviderClient {
    ...
    async fn apply(&self, args: RerankApplyArgs<'_>) -> AppResult<RerankTraceSummary> {
        let RerankApplyArgs {
            query,
            normalized_query,
            query_tokens,
...
impl LanceMemoryRepo {
    ...
    pub async fn store(&self, req: StoreRequest) -> AppResult<StoreResponse> {
        ...
        match req {
            StoreRequest::ToolStore { actor, memory } => {
                ...
                if matches!(category, Category::Behavioral) {
    ...
    pub async fn update(&self, req: UpdateRequest) -> AppResult<UpdateResponse> {
        ...
        if matches!(row.category, Category::Behavioral)
            || matches!(req.patch.category, Some(Category::Behavioral))
        {
            return Err(manual_behavioral_guidance_write_error());
        }
        ...
        let mut update_builder = table
            ...
            .column(
                "category",
                sql_string_literal(row.category.as_storage_str()),
            )
            .column("importance", row.importance.to_string())
            .column("updated_at", row.updated_at.to_string());

        let behavioral_kind_expr = row.behavioral_kind.map(behavioral_guidance_kind_to_str);
        update_builder = update_builder.column(
            "behavioral_kind",
            sql_optional_string_literal(behavioral_kind_expr),
        );
        update_builder = update_builder.column(
            "strict_key",
    ...
    pub async fn stats(&self, actor: &Actor) -> AppResult<StatsResponse> {
        ...
        for row in &rows {
            if row.category == Category::Behavioral {
                behavioral_count += 1;
            }
            *categories
                .entry(row.category.as_api_str().to_string())
                .or_insert(0_u64) += 1;
        }
    ...
    async fn recall_behavioral_guidance_internal(
        ...
        if let Some(trace) = trace.as_mut() {
            ...
            stage.metrics.insert(
                ...
                json!(match mode {
                    BehavioralRecallMode::InvariantOnly => "invariant-only",
                    BehavioralRecallMode::InvariantDerived => "invariant+derived",
                }),
            );
            trace.push(stage);
        }
        let mut filter = format!(
            "{} AND category = '{}'",
            principal_filter(&req.actor),
            Category::Behavioral.as_storage_str()
        );
        if matches!(mode, BehavioralRecallMode::InvariantOnly) {
            filter.push_str(" AND behavioral_kind = 'invariant'");
        }
        let seeds = self
            .fetch_recall_seeds(
                &table,
        ...
        let rows = ranked
            ...
            .map(|ranked_row| {
                ...
                crate::models::RecallBehavioralRow {
                    id: ranked_row.row.id,
                    text: ranked_row.row.text,
                    kind,
                    strict_key,
        ...
        let response = RecallBehavioralResponse { rows };
        let trace = trace.map(|trace| {
            trace.finish(
                RetrievalTraceKind::Behavioral,
                &req.query,
                &lexical_query,
                Some(match mode {
                    BehavioralRecallMode::InvariantOnly => "invariant-only".to_string(),
                    BehavioralRecallMode::InvariantDerived => "invariant+derived".to_string(),
                }),
            )
        });
        Ok((response, trace))
    ...
    async fn ensure_table_compatibility_and_indexes(
        ...
        if !capabilities.has_vector_column
            ...
            || !capabilities.has_behavioral_kind_column
            || capabilities.has_legacy_reflection_kind_column
        {
            return Err(AppError::backend_unavailable(
                "unsupported legacy LanceDB memory schema detected; manual migration or reset is required before startup",
            ));
        }
    ...
    async fn inspect_table_capabilities(&self, table: &LanceTable) -> AppResult<TableCapabilities> {
        ...
        let has_behavioral_kind_column = schema.field_with_name("behavioral_kind").is_ok();
        let has_legacy_reflection_kind_column = schema.field_with_name("reflection_kind").is_ok();
        match schema.field_with_name("vector") {
            Ok(field) => {
                ...
                Ok(TableCapabilities {
                    ...
                    has_behavioral_kind_column,
                    has_legacy_reflection_kind_column,
                })
            }
...
fn apply_generic_recall_filters(
    ...
    let exclude_behavioral = req.exclude_behavioral.unwrap_or(false);
    ...
    let filtered = ranked
        ...
        .filter(|row| !(exclude_behavioral && matches!(row.row.category, Category::Behavioral)))
        .filter(|row| {
            max_age_ms
                .map(|max_age| {
    ...
    ranked
        ...
        .filter(|row| {
            allowed_kinds
                .as_ref()
                .map(|allowed| {
                    let kind = row.row.behavioral_kind.unwrap_or(ReflectionKind::Derived);
        ...
        for artifact in artifacts {
            tx.execute(
                ...
                params![
                    ...
                    artifact.category.as_storage_str(),
                    artifact.importance,
                    &artifact.text,
...
fn rows_to_record_batch(rows: &[MemoryRow], vector_dimensions: usize) -> AppResult<RecordBatch> {
    ...
    let category_values: Vec<&str> = rows
        .iter()
        .map(|row| row.category.as_storage_str())
        .collect();
    ...
    let behavioral_kind_values: Vec<Option<&str>> = rows
        .iter()
        .map(|row| row.behavioral_kind.map(behavioral_guidance_kind_to_str))
        .collect();
    ...
    let columns: Vec<ArrayRef> = vec![
        ...
        Arc::new(StringArray::from(behavioral_kind_values)),
...
fn rows_from_batches(batches: &[RecordBatch]) -> AppResult<Vec<MemoryRow>> {
    ...
    for batch in batches {
        ...
        let behavioral_kind_idx = schema_index(batch, "behavioral_kind")?;
        ...
        let behavioral_kind_col =
            as_string_array(batch.column(behavioral_kind_idx), "behavioral_kind")?;
        ...
        for row_idx in 0..batch.num_rows() {
            ...
            let behavioral_kind = if behavioral_kind_col.is_null(row_idx) {
                None
            } else {
                Some(parse_behavioral_guidance_kind(
                    behavioral_kind_col.value(row_idx),
                )?)
            };
...
fn parse_behavioral_guidance_kind(raw: &str) -> AppResult<ReflectionKind> {
    match raw {
        ...
        _ => Err(AppError::internal(format!(
            "unknown behavioral kind persisted in LanceDB: {raw}"
        ))),
    }
}
...
fn memory_table_schema(vector_dimensions: usize) -> Arc<Schema> {
    ...
    Arc::new(Schema::new(vec![
        ...
        Field::new("behavioral_kind", DataType::Utf8, true),
        Field::new("strict_key", DataType::Utf8, true),
    ]))
}

fn default_strict_key(row_id: &str) -> String {
    format!("behavioral:{row_id}")
}

fn normalize_behavioral_guidance_fields(row: &mut MemoryRow) {
    if row.category != Category::Behavioral {
        row.behavioral_kind = None;
        row.strict_key = None;
        return;
    }

    if row.behavioral_kind.is_none() {
        row.behavioral_kind = Some(ReflectionKind::Invariant);
    }
    if matches!(row.behavioral_kind, Some(ReflectionKind::Invariant)) && row.strict_key.is_none() {
        row.strict_key = Some(default_strict_key(&row.id));
    }
    if matches!(row.behavioral_kind, Some(ReflectionKind::Derived)) {
        row.strict_key = None;
    }
}
...
fn is_noise_memory_text(text: &str) -> bool {
    ...
    if NOISE_EXACT_PATTERNS.contains(&lowered.as_str()) {
        return true;
    }
...
fn choose_chunk_end(
    ...
    if max_lines_per_chunk > 0 {
        ...
        for (idx, ch) in chars.iter().enumerate().take(max_end).skip(start) {
            if *ch == '\n' {
                line_breaks += 1;
