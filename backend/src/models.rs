...
#[derive(Debug, Deserialize, Serialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct RecallBehavioralRequest {
    pub actor: Actor,
    pub query: String,
    pub mode: Option<BehavioralRecallMode>,
    pub limit: u64,
    #[serde(default)]
...
#[derive(Debug, Serialize)]
pub struct RecallBehavioralResponse {
    pub rows: Vec<RecallBehavioralRow>,
}

#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct RecallBehavioralDebugResponse {
    pub rows: Vec<RecallBehavioralRow>,
    pub trace: RetrievalTrace,
}
...
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct RecallBehavioralRow {
    pub id: String,
    pub text: String,
...
fn validate_manual_behavioral_guidance_write_category(category: Option<Category>) -> AppResult<()> {
    if matches!(category, Some(Category::Behavioral)) {
...
pub fn validate_recall_behavioral_request(req: &RecallBehavioralRequest) -> AppResult<()> {
    req.actor.validate()?;
    validate_non_empty("query", &req.query)?;
    validate_limit("limit", req.limit)?;
